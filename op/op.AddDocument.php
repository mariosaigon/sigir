...
for ($file_num=0;$file_num<count($_FILES["userfile"]["tmp_name"]);$file_num++)
{
    ...
    $comment="Documento cuya reserva es válida desde el ".$fecha." sustentado en los literales ".$acum." del artículo 19 (LAIP)";
    ...
    $categoActas=$dms->getDocumentCategory(2)->getName();
			...
			foreach ($cats as $nom) 
			{
				$catsNombres[]=$nom->getName();
			}
			if(in_array($categoActas, $catsNombres))
			{
				//echo "es acta";
				$comment="Esta acta certifica que éste ente obligado no tiene ningún documento bajo condición de reserva";
			}
    //exit;
